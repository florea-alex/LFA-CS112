...
Adunarea a doua numere naturale, folosind o masina Turing cu un singur cap de citire si o singura banda.
Numere de forma: a+b; sunt transmise in fisierul "input_tm.txt", fiind codificate astfel:
zero de a ori, apoi caracterul "+", apoi zero de b ori.
Exemplu: 2+3 -> 00+000
"""
...
def loadTMFromFile(fileName):
    ...
    if len(transitions) == 0:
        ...
        print (states,sigma,gamma,transitions)
...
try:
    ...
    if errorCode == 1:
        ...
        for string in inputTM:
            ...
            if flag == 3:  # if the string is accepted by the TM
                print(f"The string \"{string}\" from \"input_tm.txt\" is accepted by the turing machine!")
                print(string, end=" = ")  # we print the initial string

                for i in range(0, len(copyString[:-3])):
                    print(copyString[i], end="")  # printing the output (the result of the addition)
                ls = [x for x in string.split("+")]

                print(" -> ", end="")
                print(len(ls[0]), end="+")  # printing the result in base 10 (normal format)
                print(len(ls[1]), end=" = ")
                print(len(copyString[:-3]))
                print("----------------------------------")
            else:  # if the input is rejected by the TM
